...
```
PC 端扫描音乐库 → 生成特征文件(.json) → 传输到手机 → 手机端导入 → 手机扫描本地音乐 → 差异比对 → 查看新增/删除/更新
```

### PC 端(WIN)

1. **启动程序**：双击 `MusicSync.exe` 或在终端执行 `python main.py`
2. **选择音乐文件夹**：在「扫描」页点击浏览，选择你的音乐目录
3. **开始扫描**：点击「开始扫描」，等待进度条完成
4. **导出特征文件**：扫描完成后，特征文件自动保存到输出目录
5. **查看结果**：切换到「结果」页，按「新增/更新/删除」分类查看差异明细

### 手机端(ANDROID)

1. **导入特征文件**：将 PC 端生成的特征文件（`pc_signature.json`）传输到手机
2. **扫描本地音乐**：在应用中导入特征文件，自动扫描手机本地音乐
3. **查看差异**：比对完成后，同样按「新增/更新/删除」分类展示

### 设置

- **扫描设置**：可调整并行线程数、大文件阈值（分块哈希）、文件扩展名过滤
- **外观设置**：支持亮色/暗色主题切换
