...
- [ ] 2.3.6 实现手动删除管理 & 二次确认
- [ ] 2.3.7 实现差异比较哈希判断开关：设置页新增「哈希值判断」开关，开启时优先用 contentHash 判定文件更新（而非仅 fileSize）；因 PC 端与手机端哈希值计算存在差异（根因待查），默认关闭并保持现有 fileSize 判断

### 2.4 PC 端设置增强
- [ ] 2.4.1 实现「哈希计算开关」：设置页新增开关，关闭时扫描跳过内容哈希，生成 `content_hash=''`、`content_hash_algo='none'`、`fingerprint_algorithms.content='none'`（与移动端对齐）
- [ ] 2.4.2 实现「哈希判定开关」：设置页新增开关，开启时差异比较优先用 `contentHash` 判定文件更新（而非仅 `fileSize`）；因两端哈希计算可能存在差异，默认关闭保持现有 `fileSize` 判定
