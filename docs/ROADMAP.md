...
### 1.1 PC 端单元测试
- [ ] 1.1.1 `test_hash_utils.py` — xxHash 正确性 & 分块策略
- [ ] 1.1.2 `test_audio_meta.py` — 正常标签 & 损坏标签 & 无标签
- [ ] 1.1.3 `test_scanner.py` — 全量扫描 & 增量扫描 & 过滤
- [ ] 1.1.4 `test_signature.py` — 生成 & 校验 & 版本兼容
- [ ] 1.1.5 `test_diff_service.py` — 新增/删除/更新/移动/未变
- [ ] 1.1.6 `test_config.py` — 加载 & 保存 & 降级
...
### 2.3 移动端增强
- [ ] 2.3.1 实现特征文件增量传输（JSON Patch 格式）
- [ ] 2.3.2 实现同步后一致性验证（本地重扫 + 比较）
- [ ] 2.3.3 实现批量文件下载（实际文件操作）
- [ ] 2.3.4 实现手动删除管理 & 二次确认
...
阶段零 (当前)     ████████████████████████  96%
  ...
  PC 服务层       ████████████████████████  100%
  PC GUI          ████████████████████████  100%
  PC 验证 (CLI)   ████████████████████░░░░   92% (需桌面 Windows 验证 GUI + 打包)
